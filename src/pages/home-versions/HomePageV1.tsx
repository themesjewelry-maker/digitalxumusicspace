...
function SectionLabel({ text }: { text: string }) {
  return (
    <motion.span
      variants={fadeInUp}
      className="inline-block text-xs font-semibold tracking-[0.2em] uppercase text-[#9C8B7A] mb-4"
    >
      {text}
    </motion.span>
  );
...
function SectionTitle({
  ...
  return (
    <motion.h2
      variants={fadeInUp}
      className={`text-3xl sm:text-4xl md:text-5xl lg:text-[56px] font-semibold tracking-tight text-[#3D2817] leading-[1.1] ${className}`}
    >
      {children}
    </motion.h2>
  );
...
function SectionDesc({
  ...
  return (
    <motion.p
      variants={fadeInUp}
      className={`text-base sm:text-lg md:text-xl text-[#9C8B7A] leading-relaxed max-w-2xl ${className}`}
    >
      {children}
    </motion.p>
  );
      ...
      <div className="max-w-[1200px] mx-auto px-6 sm:px-8">
        <div className="text-center mb-16 sm:mb-20">
          ...
          <motion.div variants={fadeInUp} className="mt-8">
            <Link
              to="/philosophy"
              className="inline-flex items-center gap-2 text-sm text-[#3D2817] hover:text-[#9C8B7A] transition-colors tracking-wide font-medium"
            >
              琴鸣教育理念
        ...
        >
          {values.map((item) => (
            <motion.div
              ...
              className="group text-center sm:text-left p-6 sm:p-8 rounded-2xl hover:bg-[#F5EDE4] transition-colors duration-500"
            >
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-[#3D2817] text-white mb-5 group-hover:scale-110 transition-transform duration-500">
                {item.icon}
              </div>
              <h3 className="text-lg font-semibold text-[#3D2817] mb-3">
                {item.title}
              </h3>
              <p className="text-sm text-[#9C8B7A] leading-relaxed">
                {item.desc}
              </p>
            </motion.div>
          ))}
...
function CourseCard({
  ...
  return (
    ...
    >
      ...
      <motion.div
        variants={scaleIn}
        className="w-full lg:w-1/2 aspect-[4/3] sm:aspect-[16/10] rounded-2xl sm:rounded-3xl overflow-hidden bg-[#F5EDE4]"
      >
        <img
          src={imageUrl}
          alt={title}
        ...
        <motion.ul variants={staggerContainer} className="space-y-3 sm:space-y-4">
          {features.map((feature) => (
            <motion.li
              key={feature}
              variants={fadeInUp}
              className="flex items-start gap-3 text-sm sm:text-base text-[#3D2817]/80"
            >
              <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5 text-[#9C8B7A] mt-0.5 shrink-0" />
              <span>{feature}</span>
            </motion.li>
          ))}
        </motion.ul>
...
function PianoSection() {
  return (
    <AnimatedSection id="piano" className="py-24 sm:py-32 md:py-40 bg-[#FDF8F3]">
      <div className="max-w-[1200px] mx-auto px-6 sm:px-8">
        <CourseCard
...
function MentorCard({ name, role, bio, imageUrl }: MentorCardProps) {
  return (
    ...
    >
      ...
      <h3 className="text-lg sm:text-xl font-semibold text-[#3D2817] mb-1">
        {name}
      </h3>
      <p className="text-xs sm:text-sm text-[#9C8B7A] font-medium mb-3 tracking-wide">
        {role}
      </p>
      <p className="text-sm text-[#9C8B7A] leading-relaxed max-w-xs mx-auto">
        {bio}
      </p>
    </motion.div>
  );
...
function MentorsSection() {
  ...
  return (
    <AnimatedSection id="mentors" className="py-24 sm:py-32 md:py-40 bg-[#FDF8F3]">
      <div className="max-w-[1200px] mx-auto px-6 sm:px-8">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
              <div>
                <label className="block text-xs font-medium text-[#9C8B7A] mb-2 uppercase tracking-wider">
                  姓名
                </label>
                <input
                  type="text"
                  ...
                  placeholder="您的称呼"
                  className="w-full px-4 py-3.5 rounded-xl bg-[#F5EDE4] border-0 text-[#3D2817] placeholder:text-[#9C8B7A]/60 text-sm focus:outline-none focus:ring-2 focus:ring-[#3D2817]/20 transition-all"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-[#9C8B7A] mb-2 uppercase tracking-wider">
                  ...
                  <button
                    ...
                    className={`px-4 py-3 rounded-xl text-sm font-medium transition-all duration-300 ${
                      form.course === opt
                        ...
                        : "bg-[#F5EDE4] text-[#3D2817] hover:bg-[#3D2817] hover:text-white"
                    }`}
                  >
                    {opt}
                  </button>
              ...
              {durationMode === "months" && (
                <div className="flex items-center gap-3">
                  <input
                    ...
                    placeholder="例如：6"
                    className="flex-1 px-4 py-3 rounded-xl bg-[#F5EDE4] border-0 text-[#3D2817] placeholder:text-[#9C8B7A]/60 text-sm focus:outline-none focus:ring-2 focus:ring-[#3D2817]/20 transition-all"
                  />
                  <span className="text-sm text-[#9C8B7A]">个月</span>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-3.5 rounded-xl bg-[#F5EDE4] border-0 text-[#3D2817] placeholder:text-[#9C8B7A]/60 text-sm focus:outline-none focus:ring-2 focus:ring-[#3D2817]/20 transition-all resize-none"
              />
            </div>
            ...
            <motion.button
              ...
              type="submit"
              disabled={loading}
              className="w-full py-4 rounded-xl bg-[#3D2817] text-white text-sm font-semibold tracking-wide hover:bg-[#5D4037] transition-colors flex items-center justify-center gap-2 disabled:opacity-60"
            >
              <Calendar className="w-4 h-4" />
              {loading ? "提交中..." : "提交预约申请"}
            </motion.button>

            <p className="text-center text-xs text-[#9C8B7A] mt-4">
...
function ContactBar() {
  return (
    <AnimatedSection className="py-16 sm:py-20 bg-[#3D2817]">
      <div className="max-w-[1200px] mx-auto px-6 sm:px-8">
        <motion.div
          variants={staggerContainer}
          initial="hidden"
...
function Footer() {
  return (
    <footer className="py-8 sm:py-10 bg-[#3D2817] border-t border-white/10">
      <div className="max-w-[1200px] mx-auto px-6 sm:px-8">
...
export default function HomePageV1() {
  return (
    <div className="min-h-screen bg-white font-sans antialiased selection:bg-[#3D2817] selection:text-white">
